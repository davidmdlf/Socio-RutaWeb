...
function getScript() {
    ...
    root_dir.getFile(script_info.name + "/" + script_info.name + ".graph", {create: false}, function(file_entry) {
        ...
        navigator.notification.alert("Imposible cargar el guión", {}, "Error");
    });
}
...
/*
 * Resize dinamically the width an height of the HTML elements in the view.
 */
function resize() {
    var mql = window.matchMedia("(orientation: portrait)");
    var calc_section_height = window.innerHeight - (3 * 16);
    var calc_answer_box_width,
            fieldset;
    if (mql.matches) {
        fieldset = document.getElementsByTagName("fieldset")[0];
        var answers_section = document.getElementById("answers_section");
        calc_answer_box_width = window.innerWidth - (7 * 16);
        if (answers_section.style.display == 'none') {
            fieldset.style.height = (calc_section_height - (2 * 16)) + "px";
        } else {
            calc_section_height /= 2;
            fieldset.style.height = calc_section_height + "px";
            answers_section.style.height = calc_section_height + "px";
        }
    }
    else {
        calc_answer_box_width = window.innerWidth / 2 - (7 * 16);
        fieldset = document.getElementsByTagName("fieldset")[0];
        fieldset.style.height = (calc_section_height - (2 * 16)) + "px";
        document.getElementById("answers_section").style.height = (calc_section_height - (1.5 * 16)) + "px";
